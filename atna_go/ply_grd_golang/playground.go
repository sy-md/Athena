...
/****************************************/
/*
a python try except and finally
*/
func errorTesting(x string) {
    /*
    once passed and next in stack if check for 
    */
    defer func() {
        if r := recover(); r != nil {
            f.Println("something went wrong")
        } else { // finally
            f.Println("the 'try except' is finshed")
        }
    }()
    // try 
    if mystr, err := f.Println(x); err != nil {
        f.Println(mystr)
        log.Fatal("%x",err)
    } else if err := ( x == "" ); err != false {
        panic("recover help") // excpet
    }
}
